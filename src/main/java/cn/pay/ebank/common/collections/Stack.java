...
/*
 * 修订记录:
 * zhyang@ebank.pay.cn 2015-04-21 23:06 创建
 *
 */
...
/**
 * Stack 接口表示后进先出（LIFO）的对象堆栈。它提供了通常的 push 和 pop 操作，以及取栈顶点的 peek 方法、测试堆栈是否为空的
 * isEmpty 方法、在堆栈中查找项并确定到栈顶距离的 search 方法,并且迭代器从栈顶开始迭代元素。
 * <p/>
...
public interface Stack<E> extends Iterable<E> {
	 ...
	 *
	 * @return 栈顶对象。
	 * @throws java.util.EmptyStackException 如果堆栈是空的。
	 */
	E peek ();
	...
	/**
	 * 返回对象在栈中的位置，以 1 为基数。如果对象 item 是栈中的一个项，该方法返回距栈顶最近的出现位置到栈顶的距离；栈中最上端项的距离为
	 * 1。使用 item 的 equals 方法比较 item 与堆栈中的项。
	 *
	 * @param item 目标对象。
	 ...
	 * <p/>
	 * <p/>
	 * 如果指定的数组能容纳 Stack 并有剩余空间（即数组的元素比 Stack 的元素多），那么会将数组中紧跟在 Stack 末尾的元素设置为
	 * null。（这对确定 Stack 的长度很有用，但只有 在调用方知道此 Stack 没有包含任何 null 元素时才可行。）
	 * <p/>
	 * <p/>
	...
	/**
	 * 返回此 Stack 的字符串表示形式。该字符串表示形式由 Stack 元素的列表组成，这些元素按其迭代器返回的顺序排列，并用方括号 ("[]")
	 * 括起来。相邻元素由字符 ", "（逗号加空格）分隔。通过 String.valueOf(Object) 可以将元素转换成字符串。
	 * 此实现会创建一个空字符串缓冲区，追加一个左方括号，然后在 Stack
	 * 上进行迭代，依次追加每个元素的字符串表示形式。每追加一个元素后（最后一个元素除外），将追加字符串
	 * ", "。最后，追加一个右括号。从字符串缓冲区获取一个字符串并返回它。
	 *
