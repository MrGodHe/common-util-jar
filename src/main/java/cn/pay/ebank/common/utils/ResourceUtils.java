...
/*
 * 修订记录:
 * zhyang@ebank.pay.cn 2015-04-21 22:11 创建
 *
 */
...
public abstract class ResourceUtils {
	...
	/**
	 * 得到 classpath 下的 resourceName 所对应的资源的一组 {@link java.net.URL} 。
	 * <p/>
	 * 该方法查找有几个优先级，从高到低依次为：
	 * <p/>
	 * <pre>
	 ...
	 * <p/>
	 * <p/>
	 * 在按照优先级查询结束后，如果没有找到资源并且 resourceName 的第一个字符不是‘/’，则会在 resourceName
	 * 前加上‘/’后再次调用一次该方法。
	 *
	 * @param resourceName 资源的名称。
	 * @param callingClass 指定一个用于在加载该 {@link Class} 的类加载器上搜索资源的 {@link Class}
	 *                     对象。
	 * @param aggregate    如果该参数为 true ，则会在找到资源后继续往更低级别找，并将找到的资源 {@link java.net.URL}
	 *                     加入返回结果中。
	 * @return classpath 下的 resourceName 所对应的资源的一组 {@link java.net.URL} 。
	 * @throws java.io.IOException 如果发生 I/O 异常。
	 */
	...
	/**
	 * 得到 classpath 下的 resourceName 所对应的一个资源的 {@link java.net.URL} 。
	 * <p/>
	 * 该方法查找有几个优先级，从高到低依次为：
	 * <p/>
	 * <pre>
	 ...
	 * @param callingClass 指定一个用于在加载该 {@link Class} 的类加载器上搜索资源的 {@link Class}
	 *                     对象。
	 * @return classpath 下的 resourceName 所对应的一个资源的 {@link java.net.URL} 。
	 */
	 ...
	 * <p/>
	 * 该方法调用 {@link #getResource(String, Class)} 得到 URL 后调用
	 * {@link java.net.URL#openStream()} 得到输入流。
	 *
	 * @param resourceName 资源的名称。
	 * @param callingClass 指定一个用于在加载该 {@link Class} 的类加载器上搜索资源的 {@link Class}
	 *                     对象。
