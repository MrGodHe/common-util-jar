...
/*
 * 修订记录:
 * zhyang@ebank.pay.cn 2015-04-21 22:03 创建
 *
 */
...
public abstract class ClassUtils {
	 ...
	 */
	public static Class<?> applicationClass(String className, ClassLoader classLoader) throws TypeNotPresentException {
		if (classLoader == null) {
	...
	/**
	 * 判定指定的 className 的类是否 ofClassName 相同,或者是否 ofClassName 的子
	 * 类/接口。该方法为在指定的类加载器中比较。
	 *
	 ...
	 */
	public static boolean instanceofType(String className, String ofClassName, ClassLoader classLoader) {
		if (classLoader == null) {
			classLoader = getDefaultClassLoader();
		}
	 ...
	 */
	public static Set<Class<?>> getClassByFilePath(String filePath) throws IllegalArgumentException, TypeNotPresentException {
	...
	public static void buildClassSetByFilePath(String filePath, Set<Class<?>> classSet) {
		buildClassSetByFilePath(filePath, getClassPath(), classSet, ClassUtils.getDefaultClassLoader());
	}
	 ...
	 * @throws TypeNotPresentException 如果有 .class 文件，但并不是 java 类文件时。
	 */
	public static void buildClassSetByFilePath(String filePath, Set<Class<?>> classSet, ClassLoader classLoader) {
		buildClassSetByFilePath(filePath, getClassPath(), classSet, classLoader);
	}
	...
	public static String getClassPath() {
		String classPath = StringUtils.checkEmptyString(ResourceUtils.getResource("", ResourceUtils.class).getFile(), "");
		if (classPath.length() > 0) {
			...
			classPath = classPath.replaceAll("/", ResourceUtils.SEPARATOR);
		}
		try {
			classPath = URLDecoder.decode (classPath, HexEncodingUtils.DEFAULT_CHARSET);
	 ...
	 *
	 * @param filePath 用于构建 {@link Class} 对象的集合的路径。
	 * @param classPath classpath 的路径，如果 filePath 指向的不与 classPath 制定的路径相同，则会抛出
	 * IllegalArgumentException ，如果为 null 则直接使用 classLoader 指定的加载器加载完整的文件路径。
	 * @param classSet 用于存放结果的集合。
	 * @throws IllegalArgumentException 如果 filePath 不是指向的 classpath 的路径。
	 * @throws TypeNotPresentException 如果有 .class 文件，但并不是 java 类文件时。
	 */
	public static void buildClassSetByFilePath(String filePath, String classPath, Set<Class<?>> classSet) throws IllegalArgumentException,
	 ...
	 * @throws TypeNotPresentException 如果有 .class 文件，但并不是 java 类文件时。
	 */
	public static void buildClassSetByFilePath(String filePath, String classPath, Set<Class<?>> classSet, ClassLoader classLoader) {
		File[] files;
		try {
			...
			if (file.isDirectory()) {
				...
				if (classPath == null) {
					...
					if (realPath.startsWith(classPath)) {
						...
						throw new IllegalArgumentException("路径 [" + realPath + "] 不是 CLASSPATH [" + classPath + "] 下的路径。");
					}
				}
			}
		}
	 ...
	 * @throws TypeNotPresentException 如果有 .class 文件，但并不是 java 类文件时。
	 */
	public static void buildClassSetByJarPath(String jarPath, Set<Class<?>> classSet, ClassLoader classLoader) {
	 ...
	 * @throws TypeNotPresentException 如果有 .class 文件，但并不是 java 类文件时。
	 */
	public static Set<Class<?>> getClassByJar(String jarPath, ClassLoader classLoader) throws IOException {
		Set<Class<?>> classSet = new HashSet<Class<?>>();
	 ...
	 * @throws TypeNotPresentException 如果有 .class 文件，但并不是 java 类文件时。
	 */
	public static void buildClassSetByJar(String jarPath, Set<Class<?>> classSet) throws IOException {
	 ...
	 * @throws TypeNotPresentException 如果有 .class 文件，但并不是 java 类文件时。
	 */
	public static void buildClassSetByJar(String jarPath, Set<Class<?>> classSet, ClassLoader classLoader) throws IOException {
	...
	/**
	 * 从一个 JarFile 里使用 {@link ClassLoader#loadClass(String)} 加载所有的 .class 为
	 * {@link Class} 对象 。
	 *
	 * @param jarFile 需要从其中加载 class 的 JarFile。
	 * @param classSet 用于存放加载的 {@link Class} 的 {@link java.util.Set} 。
	 * @param classLoader 用于加载 {@link Class} 的类加载器，对该类加载器使用
	 * {@link ClassUtils#applicationClass(String, ClassLoader)} 完成加载。
	 * @param decider 用于判定 得到的 className 的类是否需要被加载并放入 classSet 中，当判定器为 null 或者
	 * 判定结果为 true 时加载得到的 className 的类并放入 classSet 中。
	 * @see ClassUtils#applicationClass(String, ClassLoader)
	 */
	public static void parseClassByJar(JarFile jarFile, Set<Class<?>> classSet, ClassLoader classLoader, Decider<String> decider) {
	...
	private static void getAllInterfacesForClassAsSet(Set<Class<?>> interfaces, Class<?> clazz, ClassLoader classLoader) {
