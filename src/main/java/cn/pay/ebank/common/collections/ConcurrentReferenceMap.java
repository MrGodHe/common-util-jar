...
/*
 * 修订记录:
 * zhyang@ebank.pay.cn 2015-04-21 22:44 创建
 *
 */
...
import cn.pay.ebank.common.ref.*;

/**
 * 支持检索的完全并发和更新的所期望可调整并发并且键值对支持引用类型的哈希表。尽管所有操作都是线程安全的，但检索操作不必锁定，并且不
 * 支持以某种防止所有访问的方式锁定整个表。
 * <p/>
 ...
 * <p/>
 * 引用类型请参考 {@link java.lang.ref}与 {@link cn.pay.ebank.common.ref} 的描述。
 * <p/>
 ...
 * <p/>
 * 此类及其视图和迭代器实现了 {@link java.util.concurrent.ConcurrentMap} 和 {@link java.util.Iterator} 接口的所有可选 方法。
 * <p/>
 ...
 */
public class ConcurrentReferenceMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V>, AutoClearable, Serializable {
	 ...
	 */
	public ConcurrentReferenceMap (ReferenceKeyType keyReferenceType, ReferenceValueType valueReferenceType, int initialCapacity) {
		this.keyReferenceType = keyReferenceType;
		this.valueReferenceType = valueReferenceType;
	 ...
	 */
	public ConcurrentReferenceMap (ReferenceKeyType keyReferenceType, ReferenceValueType valueReferenceType, int initialCapacity, float loadFactor, int concurrencyLevel) {
		this.keyReferenceType = keyReferenceType;
		this.valueReferenceType = valueReferenceType;
	...
	/**
	 * 引用类型中的对象的另一个包装器，该包装器实现为引用类型中的对象保持 put 与 get 时用到的 hashCode 和 equals
	 * 方法的一致性。
	 *
	 * @author zhyang@ebank.pay.cn
	 */
