...
import cn.pay.ebank.common.collections.SetFromMap;
...
public class SimpleTypeConverterManager implements TypeConverterManager {
	...
	/**
	 * 注册 {@link cn.pay.ebank.common.beans.conversion} 包下的 {@link TypeConverter} 到
	 * converterManager 中。
	 * <p>
	 * 注册清单为：
	 * 
	...
	public <T> Collection<TypeConverter<T>> getTypeConverter(Class<T> targetType) {
		...
		return (Collection<TypeConverter<T>>) (m == null ? Collections.emptyList() : Collections.unmodifiableCollection(m.values()));
	}
	...
	public <S, T> TypeConverter<T> getTypeConverter(Class<S> sourceType, Class<T> targetType) {
		...
		TypeConverter<?> typeConverter = m.get (sourceType.isEnum ()?Enum.class:sourceType);
			...
			if (m == null) {
				for (Entry<Class<?>, SourceTypeConverterMap> entry : this.typeConverterMap.entrySet()) {
					if (targetType.isAssignableFrom(entry.getKey())) {
						...
						break;
					}else if (targetType.isEnum () && Enum.class.equals (entry.getKey ())) {
						m = entry.getValue();
						this.typeConverterMap.putCache(targetType, m);
						break;
					}
	...
	static class SourceTypeConverterMap extends ConcurrentReferenceMap<Class<?>, TypeConverter<?>> {
		...
		private final Set<Class<?>> notSourceTypeConverters = new SetFromMap<Class<?>, Object>(new ConcurrentReferenceMap<Class<?>, Object>(
			ReferenceKeyType.WEAK, ReferenceValueType.STRONG, 32));
	...
	static class TargetTypeConverterMap extends ConcurrentReferenceMap<Class<?>, SourceTypeConverterMap> {
		...
		private final ConcurrentMap<Class<?>, SourceTypeConverterMap> targetTypeConverterCacheMap = new ConcurrentReferenceMap<Class<?>, SourceTypeConverterMap>(
			ConcurrentReferenceMap.ReferenceKeyType.WEAK, ReferenceValueType.STRONG, 32);
		
		private final Set<Class<?>> notTargetTypeConverters = new SetFromMap<Class<?>, Object> (new ConcurrentReferenceMap<Class<?>, Object>(
			ReferenceKeyType.WEAK, ReferenceValueType.STRONG, 32));
