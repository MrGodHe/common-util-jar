...
/*
 * 修订记录:
 * zhyang@ebank.pay.cn 2015-05-13 17:13 创建
 *
 */
...
public class ClientEnvInfo implements Serializable {
	public static final String CLIENT_INFO_KEY = "ebank_client_env_info";

	public static final String DEFAULT_CLIENT_INFO_KEY = "ebank_client_env_info_default";

	public static final String CLIENT_MAC_KEY = "ebank_client_mac";
	 ...
	 */
	@Nullable
	public static ClientEnvInfo getClientEnvInfo (@Nonnull HttpServletRequest httpServletRequest, boolean createNew) {
		...
		if (session != null) {
			...
			if (clientEvnInfo == null) {
				...
				if (mac == null) {
					clientEvnInfo = (ClientEnvInfo) session.getAttribute (ClientEnvInfo.DEFAULT_CLIENT_INFO_KEY);
					//如果默认clientinfo为空，创建ClientEvnInfo对象，写入默认clientInfo
					if (clientEvnInfo == null) {
						clientEvnInfo = new ClientEnvInfo ();
						clientEvnInfo.setIp (IPUtils.getIpAddr (httpServletRequest));
