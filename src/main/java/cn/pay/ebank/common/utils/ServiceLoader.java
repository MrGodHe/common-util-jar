...
/*
 * 修订记录:
 * zhyang@ebank.pay.cn 2015-04-21 22:54 创建
 *
 */
...
import cn.pay.ebank.common.ref.FinalizableReferenceQueue;
import cn.pay.ebank.common.ref.FinalizableWeakReference;

/**
 * 一个简单的服务提供者加载设施。
 * <p>
 * 服务 是一个熟知的接口和类（通常为抽象类）集合。服务提供者
 ...
 * 这些类扩展了此服务类型，具有特定于提供者的数据和代码。提供者类
 * 通常不是整个提供者本身而是一个代理，它包含足够的信息来决定提供者是否能满足特定请求，还包含可以根据需要创建实际提供者的代码
 * 。提供者类的详细信息高度特定于服务
 * ；任何单个类或接口都不能统一它们，因此这里没有定义任何这种类型。此设施唯一强制要求的是，提供者类必须具有不带参数的构造方法，以便它们可以在加载中被实例化。
 *
 * <p>
 ...
 *
 * <p>
 * <strong>示例：</strong>假定服务类型为
 * com.example.CodecSet，它用来表示某些协议的编码器/解码器对集合。在这种情况下，它是一个具有两种抽象方法的抽象类：
 *
 * <pre>
 * <code>
...
public class ServiceLoader {
	...
	private static final ConcurrentMap<KeyWrapper, ServiceLoader> SERVICE_LOADER_MAP = new ConcurrentHashMap<KeyWrapper, ServiceLoader> ();

	private static final FinalizableReferenceQueue FINALIZABLE_REFERENCE_QUEUE = new FinalizableReferenceQueue("ServiceLoaderReferenceQueue");

	private final String rootPath;

	private final Reference<ClassLoader> classLoader;

	private final ConcurrentMap<Class<?>, Service<?>> serviceMap = new ConcurrentReferenceMap<Class<?>, Service<?>> (ConcurrentReferenceMap.ReferenceKeyType.WEAK,
			ConcurrentReferenceMap.ReferenceValueType.STRONG);
	 ...
	 *
	 * @param rootPath 资源目录 META-INF 中放置提供者配置文件根路径。
	 * @param classLoader 加载服务提供者加载器所使用的类加载器，如果为 null 则使用
	 * {@link ClassUtils#getDefaultClassLoader()} 得到的类加载器 。
	 * @return 参数对应的服务提供者加载器。
	 * @throws IllegalAccessException 如果 rootPath 为 null 。
	 */
	 ...
	 * <p>
	 * 该方法是实际为调用
	 * <code>load(serviceType, ClassUtils.getDefaultClassLoader())</code>
	 *
	...
	public <E> Service<E> load(Class<E> serviceType, ClassLoader classLoader) {
		...
		if (service == null) {
			service = new Service<E>(serviceType, classLoader, this.rootPath, this.classLoader.get());
	...
	static class KeyWrapper {
		...
		KeyWrapper(String name, ClassLoader classLoader) {
			this.name = name;
			this.classLoader = new FinalizableWeakReference<ClassLoader> (classLoader, FINALIZABLE_REFERENCE_QUEUE) {
