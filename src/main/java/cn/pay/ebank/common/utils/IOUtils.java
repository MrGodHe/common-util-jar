...
import cn.pay.ebank.common.collections.ArrayQueue;
...
public abstract class IOUtils {
	 ...
	 * null，则接受所有路径名。否则，当且仅当在路径名上调用过滤器的 {@link java.io.FileFilter#accept(java.io.File)} 方法返回
	 * true 时，该路径名才满足过滤器。
	 *
	 * @param filePath 要查找所有文件的目录路径名。
	...
	/**
	 * 返回一个抽象路径名{@link java.util.List}，这些路径名表示路径名所表示目录中的文件。
	 *
	 * @param filePath 要查找所有文件的目录路径名。
	 * @return 表示路径名所表示目录中的文件和目录的抽象路径名{@link List}。如果目录为空，则{@link java.util.List}也将为空。
	 * @throws FileOperateException 如果抽象路径名不表示一个目录，或者发生错误。
	 */
	...
	/**
	 * 返回表示路径名所表示目录中的文件和目录的抽象路径名{@link java.util.List}，这些路径名满足特定过滤器。此方法的行为与
	 * {@link #fileArray(String)} 方法相同，除了所返回{@link java.util.List}中的路径名必须满足过滤器。如果给定 filter
	 * 为 null，则接受所有路径名。否则，当且仅当在路径名上调用过滤器的 {@link java.io.FileFilter#accept(java.io.File)} 方法返回
	 * true 时，该路径名才满足过滤器。
	 *
	 * @param filePath 要查找所有文件的目录路径名。
	 * @param fileFilter 文件过滤器。
	 ...
	 * {@link java.io.FilenameFilter#accept(java.io.File, String)} 方法返回 true 时，该路径名才满足过滤器。
	 *
	 * @param filePath 要查找所有文件的目录路径名。
	 ...
	 * @return 剪切的文件/文件夹大小。
	 * @throws java.io.IOException 发生 I/O 错误时。
	 */
	 ...
	 * @param in 输入流。
	 * @param out 输出流。
	 * @param closeIn 如果为 true 将调用 {@link java.io.InputStream#close()} 。
	 * @param closeOut 如果为 true 将调用 {@link java.io.OutputStream#close()} 。
	 * @return 操作的数据大小。
	 * @throws java.io.IOException 发生 I/O 错误时。
	 */
	...
	private static int copy(File in, File out, int byteCount, Queue<File> copyFileQueue) throws IOException {
		...
		if (in.isDirectory()) { // 为目录
			...
			byteCount = byteCount
					+ copy(new BufferedInputStream(new FileInputStream(in)), new BufferedOutputStream(new FileOutputStream(out)), true, true);
			if (copyFileQueue != null) {
				copyFileQueue.offer(out);
			}
			return byteCount;
