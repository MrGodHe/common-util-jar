...
/*
 * 修订记录:
 * zhyang@ebank.pay.cn 2015-04-21 22:53 创建
 *
 */
 ...
 * >二进制名称</a>。该文件包含一个具体提供者类的完全限定二进制名称列表 ，每行一个。忽略各名称周围的空格、制表符和空行。注释字符为 '#'
 * ('&#92;u0023', NUMBER SIGN)；忽略每行第一个注释字符后面的所有字符。文件必须使用 UTF-8 编码。
 *
 * <p>
 * 如果在多个配置文件中指定了一个特定的具体提供者类，或在同一配置文件中多次被指定，则忽略重复的指定。指定特定提供者的配置文件不必像提供者本身一样位于同一个
 * jar 文件或其他的分布式单元中。提供者必须可以从最初为了定位配置文件而查询的类加载器访问；注意，这不一定是实际加载文件的类加载器。
 *
 * <p>
 * 该类是线程安全的。
...
public class Service<E> implements Iterable<E> {
	 ...
	 */
	protected Service(Class<E> serviceType, ClassLoader loadServiceClassLoader, String rootPath, ClassLoader findConfigFileClassLoader) {
		...
		while (configs.hasMoreElements()) {
			parseAndMerge(serviceType, services, serviceNames, configs.nextElement(), loadServiceClassLoader);
		}
		services.trimToSize();
		if (!services.isEmpty()) {
			this.first = services.get(0);
		} else {
			throw fail(serviceType, "[" + fullName + "]下没有服务提供者 [" + serviceType.getName() + "] 对应的服务提供者实现。");
		}
		this.services = Collections.unmodifiableList(services);
	}

	@SuppressWarnings("unchecked")
	private static <E> void parseAndMerge(Class<E> serviceType, List<E> serviceList, Set<String> serviceNames, URL url, ClassLoader classLoader) {
				...
				if (!serviceNames.contains(line)) {
					...
					try {
						...
						if (!serviceType.isAssignableFrom(c)) {
							throw fail(serviceType, "服务提供者实现 '" + line + "' 不是服务提供者的类型；该问题发生在 URL：'" + url + "' ；第 '" + num + "' 行");
						}
						serviceList.add((E) c.newInstance());
					} catch (TypeNotPresentException e) {
						throw fail(serviceType, "服务提供者实现 '" + line + "' 的类没有找到；该问题发生在 URL：'" + url + "' ；第 '" + num + "' 行", e);
					} catch (InstantiationException e) {
						throw fail(serviceType, "服务提供者实现'" + line + "' 无法创建；该问题发生在 URL：'" + url + "' ；第 '" + num + "' 行", e);
					} catch (Exception e) {
						throw fail(serviceType, "创建服务提供者实现的实例时发生错误；该问题发生在 URL：'" + url + "' ；第 '" + num + "' 行", e);
					}
				}
				num++;
			}
	...
	static ServiceConfigurationError fail(Class<?> serviceType, String msg, Throwable cause) throws ServiceConfigurationError {
		return new ServiceConfigurationError(serviceType.getName() + ": " + msg, cause);
	}

	static ServiceConfigurationError fail(Class<?> serviceType, String msg) throws ServiceConfigurationError {
		return new ServiceConfigurationError(serviceType.getName() + ": " + msg);
	}

	static ServiceConfigurationError fail(Class<?> serviceType, URL u, int line, String msg) throws ServiceConfigurationError {
